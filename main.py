...
if st.sidebar.button("🔍 About Dataset"):
    if df.empty:
        ...
        df_sample, size, info, columns, missing_values, stats, missing_percentage, outlier_df = about_df(df)
...
if st.sidebar.markdown("📈Visualise data ")     :
    if not df.empty :
        ...
        num_col,categ_col=visualise(vis)
        ...
        if show_viz:
            ...
            if type_chart=="Histogram":
                  if categ_col and num_col:
                        ...
                        col_data = vis[col].dropna()
                        max_bins=max(5, min(100, len(col_data)//10))
